...
import { FlaskService } from '../src/service';
import accountConfig from './fake-account-config';

describe('flask service deployer', () => {
    ...
    const serviceType = 'flask';
    const serviceProvisioner = new FlaskService();

    beforeEach(async () => {
        serviceParams = {
            type: serviceType
        };
        serviceContext = new ServiceContext(appName, envName, serviceName, new ServiceType('zappa', serviceType), serviceParams, accountConfig);
